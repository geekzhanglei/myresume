...
var IntroCtrl=function($scope) {
	...
	var mywheel = {
		...
		scrollFnc:function(e) {
			...
			setTimeout(function(){mywheel.addScrollListener();},600);
			$scope.loadAnimate.addPageAnimate();//加载页面动画			
		},
		init:function() {
			var _self = this;
	...
	$scope.loadAnimate = {
		addPageAnimate:function() {
			...
			$(".go_top").animate({right:"-100px"},600);
			$('.next-button').animate({opacity:1},600);
			switch(scrollIndex) {
				case 1:
				...
				case 4:
					break;
				case 5:
					$('.next-button').animate({opacity:0},500);
					$(".go_top").animate({right:"20px"},600);
					break;
				default:
					return;
			}
